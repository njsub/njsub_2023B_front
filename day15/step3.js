...
function 등록함수(){
    console.log("등록함수")
    //[1. 입력] : HTML INPUT 으로 부터 입력된 값을 가져온다.
    const content = document.querySelector('#content').value
    console.log(content)
    //[2. 처리]
    할일목록.push(content); // 
    상태목록.push( false ); // 할일의상태를 입력 받지 않고 초기값을 임의로 false  [ true = 완료 , flase = 미완 ]
    console.log(할일목록)
    console.log(상태목록)
    //[3. 출력]
    출력함수()
...
function 변경함수( 변경할인덱스 ){
    console.log("변경함수" + 변경할인덱스)
    //[1. 입력] X
    //[2. 처리] false -> ture / true - > flase
    상태목록[변경할인덱스] = !상태목록[변경할인덱스]; // 기존의 반대를 대입해서 반대값으로 수정처리
    console.log(상태목록);
    //[3. 출력]
    출력함수()
