...
document.addEventListener('DOMContentLoaded', function(){
    ...
    button.addEventListener('click', function(){
        ...
        const isEmail = function(value){
            //만약에 입력값(매개변수)에 @포함되고 @기준으로 쉬자리에 .온점이 있으면 true 없으면 false를 리턴
            ...
            /* 
                ...
                이면서
                itdanja@kakao.com => value.indexOf('@')[1].indexOf('.') > 1 => true
            */
        }
        ...
        input.addEventListener('keyup' , function(event){
            ...
            console.log(event.currentTarget.value); // 이벤트 발생한 요소으 입력값 호출
            
            const value = event.currentTarget.value;
            if(isEmail(value)){ //2번 주석의 함수 호출 => 결과는 있으면 true 없으면 false
                emailConfirm.computedStyleMap.color = `green : ${value}`
                emailConfirm.textContent = `이메일 형식이 아닙니다. : ${value}`
            }else{
                emailConfirm.computedStyleMap.color = `red : ${value}`
                emailConfirm.textContent = `이메일 형식이 아닙니다. : ${value}`
            }
